"use client";

import Link from "next/link";
import { motion } from "framer-motion";
import { Zap, Phone, Mail, MapPin, Clock } from "lucide-react";
import { siteData, serviceAreas } from "@/lib/data";

const navLinks = [
  { label: "Home", href: "/" },
  { label: "Services", href: "/services" },
  { label: "About", href: "/about" },
  { label: "FAQs", href: "/faqs" },
  { label: "Contact", href: "/contact" },
];

const serviceLinks = [
  { label: "Residential Electrical", href: "/services/residential-electrical" },
  { label: "Commercial Electrical", href: "/services/commercial-electrical" },
  { label: "Panel Services", href: "/services/electrical-panels" },
  { label: "Wiring & Rewiring", href: "/services/wiring-services" },
  { label: "Lighting Installation", href: "/services/lighting" },
  { label: "Troubleshooting", href: "/services/troubleshooting" },
];

export function Footer() {
  return (
    <footer className="bg-stone-900 text-stone-300">
      {/* Main Footer */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
          {/* Brand Column */}
          <div>
            <Link href="/" className="flex items-center gap-3 mb-6">
              <div className="w-12 h-12 bg-amber-500 rounded-sm flex items-center justify-center">
                <Zap className="w-7 h-7 text-stone-900" />
              </div>
              <div>
                <span className="text-2xl font-display text-white tracking-wide">
                  MR. WISE
                </span>
                <span className="block text-xs text-amber-500 font-medium tracking-widest">
                  ELECTRIC
                </span>
              </div>
            </Link>
            <p className="text-stone-400 mb-6">
              Professional electrical contractor serving Atlanta and surrounding areas for over 42 years. Licensed, insured, and committed to quality.
            </p>
            <div className="flex gap-4">
              <a
                href={siteData.social_links.facebook}
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-stone-800 rounded-lg flex items-center justify-center hover:bg-amber-500 hover:text-stone-900 transition-colors"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                </svg>
              </a>
              <a
                href={siteData.social_links.twitter}
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-stone-800 rounded-lg flex items-center justify-center hover:bg-amber-500 hover:text-stone-900 transition-colors"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                </svg>
              </a>
              <a
                href={siteData.social_links.instagram}
                target="_blank"
                rel="noopener noreferrer"
                className="w-10 h-10 bg-stone-800 rounded-lg flex items-center justify-center hover:bg-amber-500 hover:text-stone-900 transition-colors"
              >
                <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                  <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.052.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98C8.333 23.986 8.741 24 12 24c3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 100 12.324 6.162 6.162 0 000-12.324zM12 16a4 4 0 110-8 4 4 0 010 8zm6.406-11.845a1.44 1.44 0 100 2.881 1.44 1.44 0 000-2.881z" />
                </svg>
              </a>
            </div>
          </div>

          {/* Quick Links */}
          <div>
            <h3 className="text-lg font-display text-white tracking-wide mb-6">
              QUICK LINKS
            </h3>
            <ul className="space-y-3">
              {navLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="hover:text-amber-500 transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Services */}
          <div>
            <h3 className="text-lg font-display text-white tracking-wide mb-6">
              OUR SERVICES
            </h3>
            <ul className="space-y-3">
              {serviceLinks.map((link) => (
                <li key={link.href}>
                  <Link
                    href={link.href}
                    className="hover:text-amber-500 transition-colors"
                  >
                    {link.label}
                  </Link>
                </li>
              ))}
            </ul>
          </div>

          {/* Contact Info */}
          <div>
            <h3 className="text-lg font-display text-white tracking-wide mb-6">
              CONTACT US
            </h3>
            <ul className="space-y-4">
              <li>
                <a
                  href={`tel:${siteData.phone}`}
                  className="flex items-start gap-3 hover:text-amber-500 transition-colors"
                >
                  <Phone className="w-5 h-5 text-amber-500 mt-0.5 flex-shrink-0" />
                  <span>{siteData.phone}</span>
                </a>
              </li>
              <li>
                <a
                  href={`mailto:${siteData.email}`}
                  className="flex items-start gap-3 hover:text-amber-500 transition-colors"
                >
                  <Mail className="w-5 h-5 text-amber-500 mt-0.5 flex-shrink-0" />
                  <span>{siteData.email}</span>
                </a>
              </li>
              <li className="flex items-start gap-3">
                <MapPin className="w-5 h-5 text-amber-500 mt-0.5 flex-shrink-0" />
                <span>{siteData.address}</span>
              </li>
              <li className="flex items-start gap-3">
                <Clock className="w-5 h-5 text-amber-500 mt-0.5 flex-shrink-0" />
                <div>
                  <p>Mon-Fri: {siteData.business_hours.weekdays}</p>
                  <p>Sat-Sun: {siteData.business_hours.weekends}</p>
                </div>
              </li>
            </ul>
          </div>
        </div>

        {/* Service Areas */}
        <div className="mt-12 pt-8 border-t border-stone-800">
          <h3 className="text-sm font-display text-white tracking-wide mb-4">
            SERVICE AREAS
          </h3>
          <p className="text-sm text-stone-500">
            {serviceAreas.join(" • ")}
          </p>
        </div>
      </div>

      {/* Bottom Bar */}
      <div className="border-t border-stone-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
            <p className="text-sm text-stone-500">
              © {new Date().getFullYear()} Mr. Wise Electric. All rights reserved.
            </p>
            <div className="flex gap-6 text-sm text-stone-500">
              <Link href="/privacy" className="hover:text-amber-500 transition-colors">
                Privacy Policy
              </Link>
              <Link href="/terms" className="hover:text-amber-500 transition-colors">
                Terms of Service
              </Link>
            </div>
          </div>
        </div>
      </div>
    </footer>
  );
}
